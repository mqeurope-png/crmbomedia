...
## 8. Cierre del sprint (PR-H, 2026-06-16)

Sprint Filtros & Listas mergeado tras 13 PRs. Resumen del entregable
final.

### 8.1 PRs mergeados (orden cronológico)

| # | Branch / PR | Alcance |
|---|---|---|
| #130 | PR-A | Schema declarativo multi-entidad + generalización del motor. 5 entidades registradas. |
| #131 | PR-B | Tabla `entity_views` + endpoint genérico `/api/entities/{entity}/search` + `/search/ids`. |
| #132 | PR-C | `<EntityTable>` (TanStack) + `<EntityFilterBuilder>` (RQB) + `<EntityViewsTabs>` + `<EntityColumnConfigurator>` + sandbox. |
| #133 | PR-Cb | fix: serialize "name" computed + prune half-typed tag rules. |
| #134 | PR-Cc | fix: focus loss en input (RQB controlado→uncontrolled) + tag substring `tag_name_contains` + 9 campos schema parity. |
| #135 | PR-Cd | fix: tag picker chips siempre (RQB v8 controlElements global) + columna tags con TagChips + eager-load. |
| #136 | PR-Ce | fix: brevo-list 500 → 400, lead_score is_not_null, ENUM_NULLABLE, validate UUID; 4 pickers nuevos (User, Company, Segment, BrevoList) + bonus UX (groupByPrefix, ellipsis, deferredValue). |
| #137 | PR-Cf | fix: CompanyPicker limit 200 (cap del backend) + segment_resolver en `/api/entities/contact/search`. |
| #138 | PR-Cg | autocomplete server-side en los 5 pickers (q + debounce 300ms) + `q` en /api/users y /api/segments y /api/brevo/lists. |
| #139 | PR-E | migración /contactos al stack nuevo. Paridad funcional 1:1. |
| #140 | PR-Eb | fix: brevo lists pagesize=50 (cap real) + modal con backdrop + drag-reorder columnas con dnd-kit. |
| #141 | PR-Eb2 | redirect /marketing/listas/[id] → /contacts con filtro Brevo (Deuda #2 cerrada). |
| #142 | PR-Fa | fix: `<EntityTable>` fallback a columnas `default_visible` cuando recibe []. |
| #143 | PR-F | migración /companies al stack nuevo + bulk actions (Deuda #1 cerrada). |
| (este) | PR-H | cleanup frontend + sprint closure docs. |

### 8.2 Cumplimiento del plan original

| §4 PR plan | Estado | Notas |
|---|---|---|
| PR-A schema declarativo | ✅ | 5 entidades + 17 comparators + anti-injection. |
| PR-B entity_views + search | ✅ | Tabla compartida con discriminador, retro-compat con vistas existentes. |
| PR-C EntityTable + bulk | ✅ (parcial) | EntityTable+FilterBuilder OK. Bulk set-based NO (sigue ids-based hasta PR posterior). |
| PR-D EntityFilterBuilder + traductor RQB⇄IR | ✅ | Combinado con PR-C. |
| PR-E migrar /contacts | ✅ | Paridad 1:1 + arreglo de deuda menor (`assigned_to_me` en URL, dead `add_tag/remove_tag` removidos, columna Tags con TagChips, owner picker, etc). |
| PR-F migrar /companies | ✅ | + cierra Deuda #1 (bulk + sort + URL filters). |
| PR-G migrar /emails | ❌ → **Deuda #3** | Bandeja actual funciona; no urgente. |
| PR-H limpieza + docs | ✅ | Este PR. |

### 8.3 Decisiones cerradas durante el sprint

- **`origin_system` enum nullable comparators**: NO añadidos por ser relacional (semántica "sin external_refs" requiere diseño aparte). Documentado.
- **Brevo lists q**: paginación backend hasta 1000 listas con page_size=50 (cap real de Brevo). Substring filter en memoria.
- **Pickers server-side**: debounce 300ms con `useDebouncedValue`, top 100 sin q / top 50 con q. Patrón consistente en los 5.
- **Drag-reorder columnas**: dnd-kit (ya instalado para pipelines). `<SortableHeader>` con activación a 6px desambigua sort-click vs drag.
- **`<EntityTable>` fallback default_visible**: cuando el consumer pasa `visibleColumns=[]`, el componente se auto-defaultea desde el schema. Permite landing-by-URL sin previo localStorage.
- **/marketing/listas/[id]**: redirect puro a `/contacts?rules=…` con filtro Brevo. Renombrar/Borrar movidos al index como inline actions.

### 8.4 Deudas pendientes post-sprint

| Deuda | Detalle | Trigger sugerido |
|---|---|---|
| **#3 — Migrar /emails** | Bandeja actual con su propia paginación + filtros. EntityTable encajaría pero la UX 3-paneles (sidebar folders/labels + filters bar + thread list) requiere arquitectura específica. | Cuando alguien pida vistas guardadas / bulk avanzado en bandeja. |
| **#4 — Migrar /marketing/templates + /marketing/campaigns** | Templates es card-grid (UX cambiaría si va a EntityTable). Campaigns tiene OR%/CTR% computados de `stats_json` (JSON filtering diferido por §5 del spec). | Cuando se aborde el filtrado de JSON (`stats_json`, `custom_fields_json`). |
| **Bulk set-based** | El bulk actual enumera ids (cap 10k). Filter-native (`UPDATE … WHERE build_entity_filter(tree)` en una sentencia) queda como follow-up. | Cuando una pantalla necesite bulk sobre >10k filas. |
| **Backend legacy `/api/contacts/search` + `/api/contact-views` CRUD** | Endpoints siguen vivos por tener tests (~29 referencias en 4 archivos). El frontend ya no los usa. | Cuando alguien rebase los tests al stack nuevo. |
| **Multi-sort real** | Hoy 1 columna a la vez. El endpoint genérico aceptaría `sort=[{by,dir},…]` con cambio menor. | Cuando un operador pida ordenar por dos columnas. |
| **Métricas de campaña filtrable** | OR%, CTR% via JSON_EXTRACT o materializar columnas. | Junto con Deuda #4. |
| **Custom fields filtrable** | `custom_fields_json` whitelist + JSON filter. | Junto con Deuda #4. |
| **Generalizar `<ContactsBulkBar>`** | Hoy es contact-only. Generalizar a `<EntityBulkBar>` con acciones registradas per-entity sería el siguiente paso natural cuando otra entidad necesite bulk no-trivial. | Cuando aparezca un tercer consumer (emails / templates bulk). |
