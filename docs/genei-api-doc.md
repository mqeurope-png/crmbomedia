...
## Envíos con OTRO courier (no Genei) (rev 2026-09-26)
- Envíos hechos fuera de Genei (UPS, MRW, GLS, DSV…): etiqueta subida a mano + tracking + «📤 Marcar recogido». Módulo `app/erp/shipping_courier.py`; estado en `packing_json.envio` (sin migración): `courier` (canónico si es de la lista, si no texto libre, máx. 60), `customer_email` (mismo esquema que en el bloque `genei`), `created_by_user_id`. El tracking sigue en `Order.tracking_number`. Un pedido con `packing_json.genei.shipment_code` es de Genei y NO usa nada de esto.
- API: `POST /api/erp/orders/{id}/mark-picked-up` acepta `courier`; `PATCH /api/erp/orders/{id}/tracking` acepta `courier` (ausente = no se toca; `""` lo borra; ignorado en envíos de Genei) y responde `courier`; `GET /api/erp/orders/{id}/shipment` → `kind` (`genei` / `externo` / null), `courier`, `tracking`, `tracking_url`, `transport_status`, `picked_up_at`, `customer_email`, `suggested_courier`, `couriers`. Los items de la Cola SAT llevan `shipment_kind`, `courier`, `tracking_url`, `customer_email_status`.
- Sugerencia por formato (solo los seguros): `^1Z[0-9A-Z]{16}$` → UPS; `^0033\d{18}$` → CTT Express.
- Enlaces de seguimiento por courier: CTT Express (`app.cttexpress.com/AreaClientes/Views/Destinatarios.aspx?s=`), MRW y Correos Express (`s.correosexpress.com/c?n=`) tomados de los conectores de OCA para Odoo; UPS, FedEx, DHL y TNT, sus URL públicas de seguimiento. Sin URL (GLS, DSV, Seitrans, DB Schenker, MBE, «Otro») → el número sin enlace.
- Etiquetas: «Enviado · {courier}» / «Enviado · otro courier» (app y columna Envío de la hoja; están en `envio_vocabulary()` y la lectura de vuelta acepta el prefijo «Enviado · »). «Fecha recogido» = primera transición a `in_transit`.
