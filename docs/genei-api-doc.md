...
- **Mapeo estado Genei → `transport_status` del pedido** (fuente COMÚN para ficha, Cola SAT y hoja de Seguimiento; lo aplica el webhook y también «Actualizar estado» manual, como SYSTEM): `5`/`80`/`85`/`13`/`86` → **in_transit** (`2` «pendiente de depositar en oficina» ya NO: el transportista aún no lo tiene → bucket `ready`, como `1`); `3` → **delivered**; `9`/`10`/`14`/`15`/`78` → **incident** (incidencia de TRANSPORTE, distinta de una de pedido/taller; el texto va a `desc_incidencia`). `7`/`6`/`1`/`77`/`79` no mueven el transporte.
...
- **Estado real del envío (rev 2026-09-25)**: se guarda en `packing_json.genei` el último escaneo del transportista (`carrier_status` literal, `carrier_status_at`, `carrier_step` normalizado: `pre_transit` / `picked_up` / `in_transit` / `out_for_delivery` / `available_pickup` / `delivered` / `incident` / `unknown`), el historial (`carrier_events`, máx. 40) y `tracking_url`. Regla de transporte (`tracking.transport_target`): sin escaneo del transportista («Pendiente de entrada en red») **nunca** se pasa a «en tránsito» aunque Genei diga 5; con escaneo real (recogido/tránsito/reparto/oficina) sí, aunque Genei vaya con retraso; «entregado» del transportista → entregado; incidencia del transportista → se enseña, el transporte lo sigue moviendo Genei. Sin detalle (o texto irreconocible) → manda Genei como antes. Un escaneo guardado vale 24 h para decidir si el webhook llega sin poder leer `/tracking`.
