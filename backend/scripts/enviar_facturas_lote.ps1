...
.DESCRIPTION
  ...
  Dry-run por defecto: enseña qué haría (cliente, email, idioma, asunto, y si ya
  se envió) sin enviar nada. Con -Apply pide confirmación («escribe SI»),
...
$plan | Format-Table numero, cliente, email, idioma, asunto, estado -AutoSize | Out-String | Write-Host
...
$aEnviar | ForEach-Object { Write-Host ("  {0} → {1} ({2})" -f $_.numero, $_.email, $_.idioma) }
