...
@router.get(
    ...
    response_model=list[IntegrationSettingRead],
    responses=ERROR_RESPONSES,
...
def list_integration_settings(
    ...
    _ = current_user
    settings = crm_repository.list_integration_settings(session)
    session.commit()
    return settings
...
def get_integration_setting(
    ...
    _ = current_user
    setting = crm_repository.get_integration_setting(session, system)
    session.commit()
    if not setting:
