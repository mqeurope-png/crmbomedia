...
def update_template(
    ...
    if template is None:
        raise not_found("EmailTemplate")
    # PR-Bug-Plantillas-Permisos. Distinguimos "editar plantilla
    # global del equipo" (403) vs "editar plantilla propia que
    # casualmente está marcada como global" (OK si el flag no cambia).
    is_team_global_other = (
        template.is_global
        and template.owner_user_id != current_user.id
    )
    if not _can_edit_template(session, template, current_user):
        # Mensaje específico para el caso plantilla global del equipo
        # ajena — Bart reportó que el genérico "No tienes permiso"
        # confundía al comercial que veía la plantilla en su carpeta.
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
    ...
    # admin puede FLIPEAR el flag (en cualquier dirección).
