...
def test_manager_can_view_integration_settings(client: TestClient):
    response = client.get("/api/integration-settings", headers=auth_headers(client, "manager"))

    assert response.status_code == 200
    systems = {setting["system"] for setting in response.json()}
    assert systems == {"agilecrm", "brevo", "freshdesk", "factusol"}
    assert all(setting["enabled"] is False for setting in response.json())


def test_viewer_cannot_view_integration_settings(client: TestClient):
    response = client.get("/api/integration-settings", headers=auth_headers(client, "viewer"))

    assert response.status_code == 403


def test_admin_can_update_integration_settings_without_external_calls(client: TestClient):
    headers = auth_headers(client, "admin")

    updated = client.patch(
        "/api/integration-settings/brevo",
        json={
            "enabled": True,
            "mode": "sandbox",
            "status": "configured",
            "api_base_url": "https://api.brevo.example",
            "account_label": "Sandbox CRMBO",
            "credential_status": "configured_externally",
            "notes": "Token guardado fuera del repositorio",
        },
        headers=headers,
    )
    ...
    assert updated.status_code == 200
    assert updated.json()["system"] == "brevo"
    assert updated.json()["enabled"] is True
    assert updated.json()["credential_status"] == "configured_externally"
    assert any(entry["action"] == "update_integration_setting" for entry in audit.json())


def test_manager_cannot_update_integration_settings(client: TestClient):
    response = client.patch(
        "/api/integration-settings/agilecrm",
        json={"enabled": True},
        headers=auth_headers(client, "manager"),
    )
